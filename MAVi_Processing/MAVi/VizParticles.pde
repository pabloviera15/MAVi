void setUpParticles() {
  ...
  BConstantForce force = new BConstantForce(0, -.1f, 0);
  ...
  attr = new BAttraction(new Vec(width * .5f, height * .5f), 400, 10f);
  physics.addBehavior(attr);
  
  //lock all the Constraints (otherwise the springforce will alter the position
  VParticle a = new VParticle(-width * .5f,height, -height * .5f).lock();
  VParticle b = new VParticle(width * .5f,height, -height * .5f).lock();
  VParticle c = new VParticle(width * .5f,height, height * .5f).lock();
  VParticle d = new VParticle(-width * .5f,height, height * .5f).lock();
...
void drawParticles() {
  ...
  attr.setAttractor(new Vec(bodyRef.get(0).x, bodyRef.get(0).y, bodyRef.get(0).z));
}
